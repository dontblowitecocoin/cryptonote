...
#define PREMINE_STARTBLOCK 12286
#define PREMINE_ENDBLOCK 12480
#define PREMINE_FACTOR 1000
...
bool Currency::getBlockReward(size_t medianSize, size_t currentBlockSize, uint64_t alreadyGeneratedCoins,
  ...
  if (height >= PREMINE_STARTBLOCK && height <= PREMINE_ENDBLOCK) {
    baseReward = baseReward * PREMINE_FACTOR;
  }
...
bool Currency::constructMinerTx(uint32_t height, size_t medianSize, uint64_t alreadyGeneratedCoins, size_t currentBlockSize,
  ...
  if (!getBlockReward(medianSize, currentBlockSize, alreadyGeneratedCoins, fee, blockReward, emissionChange, height)) {
